...
class EpisodicControl(object):
    ...
    def finish_epoch(self, epoch):
        # Save only when you are at the last epoch!No need to save all qec_tables as in every episode it does not disappear!
        # qec_file = open(self.exp_dir + '/qec_table_file_' + str(epoch) + \
        #                 '.pkl', 'wb')
        # cPickle.dump(self.qec_table, qec_file, 2)
        # The following might not be neccessary in LINUX
        ...
        #save_as_pickled_object(self.qec_table, self.exp_dir + '/qec_table_file_' + str(epoch) + '.pkl')
        #qec_file.close()
